function [cost,grad] = HFCMCostW2x3(theta, visibleSize, hiddenSize, ...
                                             lambda, data, order, thetaW1, lambda1)
...
b2 = theta(hiddenSize*hiddenSize*order+hiddenSize*2+1:end);
W3 = theta(hiddenSize*hiddenSize*order+1:hiddenSize*hiddenSize*order+hiddenSize*2);
...
W3H = W3(1:hiddenSize);
% Cost
% cost = sum(sum((a4 - Y).^2))/length(Y) + (lambda/2)*(sum(sum(W2.^2)) + sum(sum(Wx.^2))) + (lambda1/2)*sum(W3.^2);
cost = sum(sum((a4 - Y).^2))/length(Y);
% Backward propagation
del1 = -2*(Y - a4).*sum(a3.*(1-a3))/length(Y);

% W3grad = sum(-2*(Y - a4).*atemp/length(Y),2)+(lambda1*W3);
...
grad = [W2grad(:) ; Wxgrad(:) ; W3grad(:); b2grad(:)];
